...
`make verify` refreshes the coreapp OpenAPI subset, compiles source/tests, runs
Ruff, ty, pytest, and the PyInstaller binary smoke. During iteration, narrower
targets are fine:
...
files you touched. Changes to release docs, package metadata, or scripts should
at least run `uv run pytest tests/test_packaging_ops.py tests/test_changelog.py`.
...
The adjacent coreapp checkout is expected at `../coreapp`. After coreapp
...
metadata bumps and release notes stay paired.
